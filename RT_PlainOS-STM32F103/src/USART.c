...
void USART1_Print64Number(unsigned long long number)
{
      unsigned char usart_save[22]={0};
			unsigned char *usart_p;
			unsigned char i=1;
...
void USART1_Print64Integer(int long long  integer)
{
	 unsigned long long  a=0;
   if(integer >= 0)
		 USART1_Print64Number((unsigned long long)integer);
	 else
	 {
		 USART1_PrintString(" -");
		 a =  (~(unsigned long long)(integer)) + 1;
		 USART1_Print64Number(a);
	 }
}
